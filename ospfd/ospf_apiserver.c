...
/* Indicates that API the server socket should be limited to local
 * connections, i.e., (127.0.0.1). */
int ospf_apiserver_local;
...
unsigned short ospf_apiserver_getport(void)
{
	 ...
	 */
	if (ospf_instance) {
		sprintf(sbuf, "ospfapi-%d", ospf_instance);
...
int ospf_apiserver_serv_sock_family(unsigned short port, int family)
{
	...
	if (ospf_apiserver_local)
		sockunion2ip(&su) = htonl(INADDR_LOOPBACK);

	/* Bind socket to optional lcoal address and given port. */
